...
impl Environment2 {
    ...
    fn ancestor(&self, distance: usize) -> Rc<RefCell<Self>> {
        let mut environment: Rc<RefCell<Environment2>> = self.opt_enclosing.as_ref().map(Rc::clone).expect("Initial environment must have an enclosing");
        for _ in 1..distance {
            let next = {
...
pub fn _count_enclosing(env: &Environment2) -> usize {
    let mut count = 0;
...
pub fn _print_env(env: &Rc<RefCell<Environment2>>, string_interner: &StringInterner) -> usize {
    let mut count = 0;
