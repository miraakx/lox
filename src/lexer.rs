...
mod tests {
    ...
    use crate::{tokens::{Token, TokenKind}, error::ConsoleErrorLogger, value::Value};
    ...
    fn test_identifiers()
    {
        assert_eq!(*tokenize("truee").get(0).unwrap().value.as_ref().unwrap(), TokenKind::Identifier( "truee".to_owned()));
        assert_eq!(*tokenize("ffalse").get(0).unwrap().value.as_ref().unwrap(), TokenKind::Identifier("ffalse".to_owned()));
        assert_eq!(*tokenize("Nil").get(0).unwrap().value.as_ref().unwrap(), TokenKind::Identifier("Nil".to_owned()));
        assert_eq!(*tokenize("ELSE").get(0).unwrap().value.as_ref().unwrap(), TokenKind::Identifier("ELSE".to_owned()));
        assert_eq!(*tokenize("whilewhile").get(0).unwrap().value.as_ref().unwrap(), TokenKind::Identifier("whilewhile".to_owned()));
    }
    */
